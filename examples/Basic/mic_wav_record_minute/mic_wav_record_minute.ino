 ...
 */
 ...
 // Add new variables for chunked recording
 ...
 void loop(void)
 {
     ...
     // Press BtnA to start/stop recording
         ...
         if (chunks_recorded < TOTAL_CHUNKS) {
             ...
             if (M5Cardputer.Mic.record(data, record_length, record_samplerate)) {
                 ...
                 if (writeChunkToFile(data, record_length)) {
                     ...
                     int progress = (chunks_recorded * 100) / TOTAL_CHUNKS;
                     M5Cardputer.Display.drawString(String(progress) + "%", 180, 3);
                 }
             }
         } else {
             finishRecording();
 ...
 bool startWAVFile() {
     ...
     // Write WAV header with final size pre-calculated
     WAVHeader header;
